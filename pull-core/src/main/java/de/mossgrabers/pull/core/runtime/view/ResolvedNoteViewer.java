...
/** One target-fenced note viewer and the drum-controller behavior attached to it. */
record ResolvedNoteViewer (ControllerNoteView layout, boolean automaticRollAttached)
{
    static ResolvedNoteViewer resolve (final ControllerSnapshot snapshot, final boolean noteDestinationPending)
    {
        ...
        if (!selected.exists () || !sameTarget (selected, preference))
            return new ResolvedNoteViewer (ControllerNoteView.NONE, false);
        ...
        return new ResolvedNoteViewer (layout, drumControllerAttached && (!layout.isPresent () || layout == ControllerNoteView.DRUM_PAD));
    }
