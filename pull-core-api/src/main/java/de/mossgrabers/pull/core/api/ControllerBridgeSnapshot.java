 ...
 * @param mappedPadLights Bitwig manual-mapping feedback for the four drum control pads
 * @param master Current project and Master-page state
 * @param project Lightweight current-project state
 */
public record ControllerBridgeSnapshot (TransportSnapshot transport, SelectedTrackSnapshot selectedTrack, ControllerLayoutSnapshot layout, NoteViewSnapshot noteView, NoteRepeatSnapshot noteRepeat, DrumContextSnapshot drum, ParameterBridgeSnapshot parameters, MappedPadLightsSnapshot mappedPadLights, MasterSnapshot master, ProjectSnapshot project)
{
    private static final ControllerBridgeSnapshot EMPTY = new ControllerBridgeSnapshot (TransportSnapshot.empty (), SelectedTrackSnapshot.empty (), ControllerLayoutSnapshot.empty (), NoteViewSnapshot.empty (), NoteRepeatSnapshot.empty (), DrumContextSnapshot.empty (), ParameterBridgeSnapshot.empty (), MappedPadLightsSnapshot.empty (), MasterSnapshot.empty (), ProjectSnapshot.empty ());
    ...
    public ControllerBridgeSnapshot
    {
        ...
        mappedPadLights = Objects.requireNonNull (mappedPadLights, "mappedPadLights");
        master = Objects.requireNonNull (master, "master");
        project = Objects.requireNonNull (project, "project");
    }


    /** Compatibility constructor for snapshots without mapped pad-light feedback. */
    ...
    {
        this (transport, selectedTrack, layout, noteView, noteRepeat, drum, parameters, MappedPadLightsSnapshot.empty (), master, project);
    }
    ...
    {
        this (transport, selectedTrack, layout, NoteViewSnapshot.empty (), NoteRepeatSnapshot.empty (), drum, parameters, MappedPadLightsSnapshot.empty (), master, project);
    }
    ...
    {
        this (transport, selectedTrack, layout, NoteViewSnapshot.empty (), NoteRepeatSnapshot.empty (), drum, parameters, MappedPadLightsSnapshot.empty (), master, ProjectSnapshot.empty ());
    }
    ...
    {
        this (transport, selectedTrack, layout, NoteViewSnapshot.empty (), NoteRepeatSnapshot.empty (), drum, parameters, MappedPadLightsSnapshot.empty (), MasterSnapshot.empty (), ProjectSnapshot.empty ());
    }
