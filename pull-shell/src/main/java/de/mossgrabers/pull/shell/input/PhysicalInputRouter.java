 ...
 * never leak into a legacy command which did not receive the press. Motion is bounded to one
 ...
 * The controller setup can pass a button's complete legacy dispatch or a continuous control's
...
public final class PhysicalInputRouter<C>
{
    ...
    private final Map<PhysicalInputAddress<C>, InputRoute> gestureRoutes;
    ...
    {
        this (registry, routeResolver, eventSink, System::nanoTime);
    }
    ...
    PhysicalInputRouter (final PhysicalControlRegistry<C> registry, final BiFunction<? super C, ? super InputKind, InputRoute> routeResolver, final Consumer<? super PhysicalInputEvent<C>> eventSink, final LongSupplier nanoTime)
    {
        ...
        this.gestureRoutes = new HashMap<> (registry.capacity ());
     ...
     * ownership. The legacy callback runs exactly once for NONE and OBSERVE, and never for
     ...
     * @param legacyCommand Existing stable controller command
     * @return The route applied to this sample
     */
    public InputRoute route (final C control, final InputKind kind, final InputPhase phase, final long value, final Runnable legacyCommand)
    {
        Objects.requireNonNull (phase, "phase");
        Objects.requireNonNull (legacyCommand, "legacyCommand");
        ...
        return kind.isEdge () ? this.routeEdge (input, phase, value, legacyCommand) : this.routeMotion (input, phase, value, legacyCommand);
    }
    ...
    {
        return this.gestureRoutes.size ();
    }


    private PhysicalInputEvent<C> newEvent (final PhysicalInputAddress<C> input, final InputPhase phase, final long value)
    {
        if (this.nextSequence == Long.MAX_VALUE)
            throw new IllegalStateException ("Physical input sequence exhausted");
        return new PhysicalInputEvent<> (this.nextSequence++, this.nanoTime.getAsLong (), input.control (), input.kind (), phase, value);
    }


    private InputRoute routeEdge (final PhysicalInputAddress<C> input, final InputPhase phase, final long value, final Runnable legacyCommand)
    {
        final InputRoute route;
        if (phase == InputPhase.BEGIN)
        {
            final InputRoute existing = this.gestureRoutes.get (input);
            route = existing == null ? this.resolveRoute (input) : existing;
            if (existing == null)
                this.gestureRoutes.put (input, route);
        }
        else
            route = this.gestureRoutes.getOrDefault (input, InputRoute.NONE);

        final PhysicalInputEvent<C> event = this.newEvent (input, phase, value);
        try
        {
            this.deliverEdge (route, event, legacyCommand);
            return route;
        }
        finally
        {
            if (event.phase () == InputPhase.END)
                this.gestureRoutes.remove (input);
        }
    }


    private InputRoute routeMotion (final PhysicalInputAddress<C> input, final InputPhase phase, final long value, final Runnable legacyCommand)
    {
        final InputRoute route = this.resolveRoute (input);
        if (route != InputRoute.EXCLUSIVE)
            legacyCommand.run ();
        if (route != InputRoute.NONE)
            this.coalesce (input, this.newEvent (input, phase, value));
        return route;
    }


    private void deliverEdge (final InputRoute route, final PhysicalInputEvent<C> event, final Runnable legacyCommand)
    {
        if (route != InputRoute.EXCLUSIVE)
            legacyCommand.run ();
        if (route != InputRoute.NONE)
    ...
    private void coalesce (final PhysicalInputAddress<C> input, final PhysicalInputEvent<C> event)
    {
        if (event.kind ().sumsRelativeValues ())
        {
            ...
            final long value = previous == null ? event.value () : Math.addExact (previous.value (), event.value ());
            if (value == 0)
            {
