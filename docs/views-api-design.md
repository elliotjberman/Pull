...
An observed stopped-to-playing launcher transition anchors a clip position that advances from
transport read-back and wraps through the observed loop. Core divides that position by the active
timeline resolution and makes only the resulting selected pad blink; selectable unselected pads
and the rest of the selected range remain steady, while padding remains off. A target change or
transport discontinuity clears the phase and therefore the blink until a later observable launch.
