...
| `CLIP_TIMELINE` | exact selected audio-clip target, loop start/length, nonshrinking selectable extent, optional tracked playback position, and color | One startup-created bounded selection-following cursor clip. API 21 has no audio-clip position property, and the installed API 25 reference adds none, so phase is published only after an observed stop-to-play transition, advances from sampled transport read-back, and fails closed on target or transport discontinuity. Publication and range effects require agreement with the private selected target's generation/channel identity and the cursor clip's track/scene identity. |
