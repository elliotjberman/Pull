...
the bounded Core API 43 controller bridge
described below: normalized Push command input, explicitly requested
transport/selected-track/Session-bank/layout/clip-timeline/note-view/note-repeat/drum/parameter/semantic-mapping/Master read-back, and typed
...
Because API 43 and its bridge are parent-loaded, installing this
...
## Installed API 43 bounded capability canopy

Core API 43 installs a broad input seam and a deliberately finite Bitwig state/effect bridge during
...
| `CLIP_TIMELINE` | exact selected audio-clip target, loop start/length, nonshrinking selectable extent, optional tracked playback position, and color | One startup-created bounded selection-following cursor clip. API 21 has no audio-clip position property, live audio clips do not publish the generic playing step, and the installed API 25 reference adds no position. The eager cursor retains low-rate launcher playback edges while timeline snapshots are unrequested; an observed selected-track stopped edge survives the cursor's same-track scene retarget and is consumed by the first exact playing target. Phase advances from sampled transport read-back. Up to 16 exact project/track/scene and clip-geometry transport offsets survive selection, core, and shell restarts in one hidden global preference created during driver initialization. The already-requested transport clock advances retained offsets across arranger-loop wraps while Clip Timeline is hidden; stop and other transport-discontinuity read-back invalidate them. A playing target with no prior exact anchor fails closed. Publication and range effects require agreement with the private selected target's generation/channel identity and the cursor clip's track/scene identity. |
...
API 43 can request absolute transport state and values; selected-track activation, group expansion,
...
selection; and an exact start/length/play range for the authoritative selected audio clip. The four mappable drum-control pads emit no core effect: Bitwig's semantic
...
This remains a capability canopy, not a mirror of an unbounded Bitwig project. API 43 does not add
...
track-window identity; API 43 does not advertise parameter-only send slots without that alignment.
...
Output remains narrower than input in API 43. This is the canonical installed-output inventory:

| Lane | Installed ownership |
| --- | --- |
| RGB lights | Any registered Push button or physical grid pad can be explicitly claimed by a core view. One generic `ControllerLight` supplies a base color plus an optional tempo-clocked alternate. Native slow/fast requests use Push's firmware blink channels. Exact musical requests additionally carry a beat-cycle, alternate-phase boundary, transport offset, and core-authored current phase; stable admits them only for grid pads, resolves the colors, and transmits the visible phase without owning cadence or waveform policy. The debugger receives one atomic complete-grid semantic frame at the committed-output boundary even when the physical palette cache suppresses a redundant MIDI send. The shell validates the physical registry, gives an explicit owner precedence, and otherwise preserves the exact stable supplier. Current core owners include the complete Clip Timeline grid/scenes, the sixteen drum-play, eight physical drum-fill, four drum-rate, and four mappable-control lights, global Play/Record and selected-track Mute/Solo, Session Stop Clip while a Session view is active, and both Master rows while the Master-controls facet is active. Drum-fill semantic action identities remain separate from those physical outputs, which are present only while the authoritative Drum layout is engaged. |
...
Once API 43 is installed, new mappings, modes, gestures, and effects composed only from these exact
...
The API 43 snapshot contains revision, monotonic time, shell capabilities, the explicitly subscribed
...
Core API 43's type hierarchy retains logical timer effects for the proposed contract, but they are
...
| Request or stop requesting an existing API 43 bridge subscription | Core reload |
| Select a different installed API 43 parameter bank or remap an installed bank's encoder turns | Core reload |
| Output policy or physical projection inside the canonical API 43 installed inventory | Core reload |
...
- Unrequested API 43 bridge domains publish typed empty values without domain snapshot construction
