...
| `CLIP_TIMELINE` | exact selected audio-clip target, loop start/length, nonshrinking selectable extent, optional tracked playback position, and color | One startup-created bounded selection-following cursor clip. API 21 has no audio-clip position property, and the installed API 25 reference adds none. The eager cursor retains only low-rate launcher playback edges while timeline snapshots are unrequested; an observed selected-track stopped edge survives the cursor's same-track scene retarget and is consumed by the first exact playing target. Phase then advances from sampled transport read-back and fails closed on track, target, or transport discontinuity. Publication and range effects require agreement with the private selected target's generation/channel identity and the cursor clip's track/scene identity. |
