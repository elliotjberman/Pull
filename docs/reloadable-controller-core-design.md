...
| `CLIP_TIMELINE` | exact selected audio-clip target, loop start/length, nonshrinking selectable extent, authoritative launcher-playing state, and color | One startup-created bounded selection-following cursor clip. Publication and range effects require agreement with the private selected target's generation/channel identity and the cursor clip's track/scene identity. |
