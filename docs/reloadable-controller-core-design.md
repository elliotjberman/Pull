...
are implemented. The current working tree installs the bounded Core API 27 controller bridge
...
The drum-fill shell uses a single-active replacement barrier, while one stable Note-performance
lifecycle routes the Pads input directly to the aligned selected track. The Master page's two button rows and
eight-column display now have complete output arbitration; other general Push output does not.
Because API 27 and its bridge are parent-loaded, installing this
...
### Selected-track observation and Note-performance routing
...
The core resolves the selected track's viewer into one `DesiredNotePerformance`: the installed
layout and, for a musical note-capable target, a generation/channel-fenced direct route. One stable
`NotePerformanceHost` owns the transition. It validates the target and submits the direct route
before applying an entering layout; on normal exit it commands the real Track/Session neutral
layout, waits for held pad and sustain lifecycles, neutralizes parent-owned raw MIDI state, and
submits removal. A selection-identity disagreement neutralizes and removes immediately, then
quarantines a new submission until the selected target and viewer agree and musical input is idle.
Core replay reasserts the layout without route churn; core invalidation neutralizes and removes
immediately. Bitwig exposes no route-attachment read-back, so this is ordered topology submission,
not an acknowledgement claim. The stable shell advances a layout generation only when its complete
visible layout state changes; a destination request cannot be retired by the stale layout sampled
before that request. There is no fallback to `All Inputs`.
...
## Installed API 27 bounded capability canopy

Core API 27 installs a broad input seam and a deliberately finite Bitwig state/effect bridge during
...
musical data still follows the stable `DesiredNotePerformance` route. Musical routing therefore
...
This remains a capability canopy, not a mirror of an unbounded Bitwig project. API 27 does not add
...
track-window identity; API 27 does not advertise parameter-only send slots without that alignment.
...
Output remains narrower than input in API 27. This is the canonical installed-output inventory:
...
| Note performance | One complete replayable installed layout plus target-fenced selected-track note route; one stable lifecycle owner orders topology submission, activation, musical-idle-gated removal, mismatch quarantine, and failure cleanup. |
...
Once API 27 is installed, new mappings, modes, gestures, and effects composed only from these exact
...
The API 27 snapshot contains revision, monotonic time, shell capabilities, the explicitly subscribed
...
clip bindings, controller workspace/actions, the composed Note-performance lifecycle, note-repeat ownership, parameter-bank
...
Core API 27's type hierarchy retains logical timer effects for the proposed contract, but they are
...
| Request or stop requesting an existing API 27 bridge subscription | Core reload |
| Select a different installed API 27 parameter bank or remap an installed bank's encoder turns | Core reload |
| Output policy inside a lane in the canonical API 27 installed-output inventory | Core reload |
...
- Unrequested API 27 bridge domains publish typed empty values without domain snapshot construction
