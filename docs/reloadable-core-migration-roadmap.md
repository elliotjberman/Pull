...
API 42 owns Mute/Solo as one persistent selected-track view and Stop as part of `SessionView`.
...
API 42 publishes stable identities, names, semantic channel types, generation, offsets, and basic authoritative state for
...
API 42 installs the bounded visible Session bank's track identities/names/types, track/scene offsets,
