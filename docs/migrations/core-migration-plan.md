...
| --- | --- | --- |
| Project Macro touch and Delete reset | Exact parameter-touch lifetime, automation/configuration observations and effects | Implemented; prior representative reset/touch checks pass; API 46 mapped Boolean macro write/read-back and release pass in 202arp3. Continuous precision and all reset variants remain unverified. |
| Track/Mix and Master touches | Reuse touch mechanism; include send-enabled and page-specific variants | Implemented; representative live volume/pan/send and Master touch checks passed; API 46 selection/return and retained Track touch under legacy Device pass. |
...
| Track/Mix menus and normal footer | Named selected-track/send parameters, I/O, current-bank state, exact effects | Normal and VS Track implemented; representative parameters passed live; reentrant cleanup correction passes API 46 live footer selection/return and held-input reload. |
