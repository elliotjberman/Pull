...
| Volume/Pan pages | Named current-bank parameters, exact owner metadata, shared footer, preferences, generic page registry | Implemented; expanded offline package gate in progress; live pending |
| Send pages 1–8 | Eight requested columns across eight current tracks, exact owner/position fences, shared menu/footer/touch machinery | Implemented with stable SendMode deletion; focused tests passed, expanded package and live pending |
| Four-arrow navigation | Independent current-bank track/scene/cursor fence and installed inert arrow footprint | Core-owned for registered pages and VS; full Session legacy-page arrows and page/sequencer buttons remain declared frozen behavior; expanded gate in progress |
| Global Track/Mix button | Raw mode/history read-back, generic mode effects, controller preferences, semantic action barrier | Complete gesture and feedback policy implemented; expanded gate in progress; live pending |
...
| Metronome/Automation and temporary settings pages | Generic SELECT/TEMPORARY/RESTORE, transport settings, raw automation mode/reset, exact parameters | Implemented with action/feedback and observer ownership; expanded gate in progress; live pending |
| Frame and Master entry | Generic application UI context, thirteen observed flags, native primitives, retained page/background composition | Implemented with delayed entry/restore acknowledgement and full row/display ownership; expanded gate in progress; live pending |
| Accent | Observed enabled/velocity preferences, core-authored Drum velocity map, generic page and input admission | Implemented; focused verification in progress; summed-motion clamp deviation recorded in the shortcut ledger |
...
application-UI effects 1. This build has not been installed live.
...
- The expanded deprecation-enabled package passed **823 tests** (385 core, 11 publication,
  ...
  remains in progress, so this is a verification checkpoint, not final acceptance or live proof.
